...
- Range-heavy media fixtures: request first, middle, and suffix byte ranges from
  known media CIDs and verify `206`, `Content-Range`, `Content-Length`, and
  bounded memory. The harness now has opt-in first/middle/suffix/HEAD coverage
  for the `ipfs.tech` developers hero image; stable audio/video CIDs are still
